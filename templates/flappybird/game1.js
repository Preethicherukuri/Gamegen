    ...
    switch (season) {
      ...
      case 'spring':
        ...
        flowers.forEach(tree => {
          ...
          // Main central circle
          ctx.beginPath();
          ctx.arc(tree.x + 7, tree.y - 20, 25, 0, Math.PI * 2);
          ctx.fill();

          // Left puff
          ctx.beginPath();
          ctx.arc(tree.x - 3, tree.y - 15, 20, 0, Math.PI * 2);
          ctx.fill();

          // Right puff
          ctx.beginPath();
          ctx.arc(tree.x + 23, tree.y - 15, 20, 0, Math.PI * 2);
          ctx.fill();

          // Top puff
          ctx.beginPath();
          ctx.arc(tree.x + 7, tree.y - 40, 18, 0, Math.PI * 2);
          ctx.fill();

          // Fixed flowers on canopy (no rotation)
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (difficultySelected && !paused) velocity = jump;
  });
